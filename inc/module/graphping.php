...
if($id){
$sqlpingtoday = $db->SimpleWhile('SELECT * FROM `pingstats` WHERE system = '.$id.' AND datetime  >= curdate() ORDER BY datetime ASC');
if(count($sqlpingtoday)>3){
	foreach($sqlpingtoday as $ping){
		$clocks = new DateTime($ping['datetime']);
		$DATA['x'][] = date_format($clocks, 'H:i');
		$DATA[0][] = $ping['time'];
		$DATA[1][] = ($ping['status']==1?1:0);
		$DATA[2][] = 0;
		$DATA[3][] = 0;
	}
