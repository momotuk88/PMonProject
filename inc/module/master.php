...
$getid = $getid ?? null;
$init = $init ?? null;
$getid = isset($_POST['getid']) ? Clean::int($_POST['getid']): null;
$init = isset($_POST['init']) ? Clean::text($_POST['init']): null;
switch($init){
	case 1: // get list onu zte c300
		$dataSwitch = $db->Fast('switch','*',['id'=>$getid]);
...
$ajaxscript = '<script>startmaster('.$dataSwitch['id'].',1);</script>';
}
$tpl->load_template('master/main.tpl');
$tpl->set('{oltname}',$dataSwitch['place'].' '.$dataSwitch['inf'].' '.$dataSwitch['model']);
$tpl->set('{result}',$tplRes);
$tpl->set('{scriptmaster}',$ajaxscript);
$tpl->set('{olt}',$id);
