...
class CDATA_1108 { 
    ...
    private $configapionugpon = 'dist,name,mac,status,rx,tx,eth,model,vendor,reason,temp';
    private $filter = true;
	...
	*/
	public function Load(){
		...
		$listinface = $this->deviceoid[$this->id]['onu']['listmac']['epon']['oid'];
		...
		if($listonu){
			...
			$indexOnu = explodeRows(str_replace('.'.$listinface.'.1.','',$listonu));
			if(is_array($indexOnu)){
				foreach($indexOnu as $io => $eachsig) {
					...
					if(isset($line[0]) && isset($line[1])) {
						...
						$result[$io] = array('do' => 'onu','id'=>$this->id,'mac'=>ClearDataMac($this->prepareResult($TempName)['name']),'pon'=>'epon','inface'=>'0/'.str_replace('.',':',$line[0]),'types'=>$this->primary,'keyonu'=> trim($dataMatch[2]),'keyport'=> trim($dataMatch[1]),'portolt'=> trim($dataMatch[1]));
					}	
				}
				if(is_array($result)){
			...
			if(is_array($IndexEponPort)){
				...
				foreach ($IndexEponPort as $idPort => $ValuePort) {				
					...
					if(!empty($infPort[0]) && !empty($infPort[1])){
						...
						$listPort[$idPort] = array('id' =>trim($infPort[0]),'typeport' => getTypePort($getinf),'name' => $getinf);
					}				
				}
				$data['ports'] = $listPort;
			}
		...
		if(is_array($data['ports'])){
			foreach($data['ports'] as $idPonport => $valuePon){
				if(preg_match('/pon/i',$valuePon['name'])) {
					...
					$listPon[$idPonport]['typeport'] = 'epon';
				...
				if(preg_match('/sfp/i',$valuePon['typeport'])) {
					preg_match('/0\/(\d+)/',$valuePon['name'],$mat);
					$listPortget[$idPonport]['name'] = 'GE 0/'.$mat[1].'';
					$listPortget[$idPonport]['sort'] = $mat[1];
					$listPortget[$idPonport]['typeport'] = 'ge';
					$listPortget[$idPonport]['llid'] = $valuePon['id'];
				}
			}
			if(is_array($listPon)){
