...
if(isset($getONT['idonu'])){
$getOLT = $db->Fast('switch','*',['id' => $getONT['olt']]);
if(isset($getOLT['netip']) && isset($getOLT['class'])){
...
if($support){
	...
	if(is_array($resultONT)){
		if(isset($resultONT['adminstatus'])){
		...
		if(isset($getONT['added'])){
			echo ont_label($lang['registers'],$getONT['added']);
		}			
		if($resultONT['status']==1 && isset($resultONT['online'])){
		...
		if(isset($resultONT['reason']))
			echo ont_label(($resultONT['status']==1?$lang['rereason']:$lang['reason']),tplreason($lang[$resultONT['reason']],$resultONT['reason']));		
		// ZTE C3xx		
		if(isset($resultONT['wan']) && $resultONT['status']==1 && $getOLT['oidid']==7){
			echo ont_label($lang['wanport'],'<img src="../style/img/'.(isset($resultONT['wanportzte']['txt']) ? $resultONT['wanportzte']['img'] : 'eth'.$resultONT['wan']).'.png" class="onueth">');
			if(is_array($resultONT['wanportzte']) && isset($resultONT['wanportzte']['txt']))
				echo ont_label($lang['typeport'],$resultONT['wanportzte']['txt']);
		}
		// HUAWEI 56xx
		if($getOLT['oidid']==14){
			if(isset($resultONT['onuerror']))
				echo ont_label($lang['coutporterr'],'<font color=red>'.$resultONT['onuerror'].'</font>');			
			if(isset($resultONT['linervlan']))
				echo ont_label('inner-vlan',$resultONT['linervlan']);
			if(isset($resultONT['uservlan'])){
		...
		// ZTE C3xx
		if(isset($resultONT['name']) && $getOLT['oidid']==7){
		...
		// ZTE C3xx		
		if(isset($resultONT['note']) && $getOLT['oidid']==7){
		...
		if(isset($resultONT['lastrx']))
			echo ont_label($lang['lastrx'],signalTerminal($resultONT['lastrx']));
		if(isset($resultONT['tx']) && $resultONT['status']==1)
			echo ont_label('TX ',signalTerminal($resultONT['tx']));		
		if(isset($resultONT['temp']) && $resultONT['status']==1)
		...
		if($getOLT['oidid']==1){
			if(isset($resultONT['pvid']))
				echo ont_label($lang['bdcom_vlan'],$resultONT['pvid'].(isset($getOLT['snmprw'])?'<span id="edit-vlan-'.$getONT['idonu'].'" ></span><span class="ont-btn" id="btn-edit-vlan-'.$getONT['idonu'].'" onclick="bdcomvlanonu_ajax('.$getONT['olt'].','.$getONT['idonu'].',\'bdcomformeditonu\')">'.$lang['edit'].'</span>':''));		
			if(isset($resultONT['uptime']))
				echo ont_label($lang['uptime'],convertuptime($resultONT['uptime']));		
			if(isset($resultONT['typereg']))
				echo ont_label($lang['typereg'],$resultONT['typereg']);
			// BDCOM EPON - список МАС за ону
			if($getOLT['oidid']==1 && isset($getOLT['snmprw']) && !$getOLT['username'] && $USER['class']>=5){
				echo'<span class="snmpreboot" onclick="rebootonu_ajax('.$getONT['olt'].','.$getONT['idonu'].',\'reboot\')">'.$lang['reboot'].' SNMP</span>';
			}		
		}
		// C_DATA
		if($getOLT['oidid']==15 && isset($getOLT['snmprw']) && $USER['class']>=5){
